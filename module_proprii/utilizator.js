...
class Utilizator{
    ...
    constructor({id, username, nume, prenume, email, parola, rol, culoare_chat="black", poza, data_nastere,tema}={}) {
        this.id=id;
        ...
        //vreau sa transform rol dintr-un string (ex "admin","comun") intr-o instanta a clasei rol admin
        if(this.rol)
    ...
    */
    ...
    salvareUtilizator(){
        ...
        AccesBD.getInstanta(Utilizator.tipConexiune).insert({tabel:Utilizator.tabel,
            ...
            `<h1>Salut!</h1><p style='color:blue'>Username-ul tau este ${utiliz.username}.</p> <p><a href='http://${Utilizator.numeDomeniu}/confirmare_mail/${token}/${utiliz.username}'>Click aici pentru confirmare!!!</a></p>`,
            )


            utiliz.trimiteMail("Salut, "+utiliz.prenume+" "+utiliz.nume+" .","Bine ai venit!",`<p>Te-ai inregistrat pe <a href='http://${Utilizator.numeDomeniu}'>Knit Wit</a> cu username-ul <i>${utiliz.username}</i>.</p>`,
            )
...
//xjxwhotvuuturmqm
    ...
    static getUtilizDupaUsername (username,obparam, proceseazaUtiliz){
        if (!username) return null;
        let eroare=null;
        AccesBD.getInstanta(Utilizator.tipConexiune).select({tabel:"utilizatori",campuri:['*'],conditii:[[`username=$1`]]}, function (err, rezSelect){
            let u = null;
            if(err){
